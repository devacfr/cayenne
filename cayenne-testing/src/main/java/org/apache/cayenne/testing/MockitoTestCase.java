...
public abstract class MockitoTestCase extends Assert {
     ...
     * is used in all verification errors.
     * <p>
     * Beware that naming mocks is not a solution for complex code which uses
     * too many mocks or collaborators. <b>If you have too many mocks then
     * refactor the code</b> so that it's easy to test/debug without necessity
     * of naming mocks.
     * <p>
     * <b>If you use &#064;Mock annotation then you've got naming mocks for
     * free!</b> &#064;Mock uses field name as mock name. {@link Mock Read
     * more.}
     * <p>
     *
     * See examples in javadoc for {@link Mockito} class
    ...
    /**
     * Creates a spy of the real object. The spy calls <b>real</b> methods
     * unless they are stubbed.
     * <p>
     * Real spies should be used <b>carefully and occasionally</b>, for example
     ...
     *
     * 1. Sometimes it's impossible to use {@link Mockito#when(Object)} for
     * stubbing spies. Example:
     *
     * <pre>
     * List list = new LinkedList();
     * List spy = spy(list);
     ...
     * bottom line is: when you spy on real objects + you try to stub a final
     * method = trouble. What will happen is the real method will be called *on
     * mock* but *not on the real instance* you passed to the spy() method.
     * Typically you may get a NullPointerException because mock instances don't
     ...
     * </pre>
     * <p>
     * Some users find stub() confusing therefore {@link Mockito#when(Object)}
     ...
     * then it should not be stubbed. Not convinced? See <a
     * href="http://monkeyisland.pl/2008/04/26/asking-and-telling">here</a>.
     *
     * @param methodCall
     *            method call
     ...
     * {@link ArgumentCaptor} or {@link ArgumentMatcher} to find out other ways
     * of matching / asserting arguments passed.
     * <p>
     * Although it is possible to verify a stubbed invocation, usually <b>it's
     * just redundant</b>. Let's say you've stubbed foo.bar(). If your code
     * cares what foo.bar() returns then something else breaks(often before even
    ...
    /**
     * Smart Mockito users hardly use this feature because they know it could be
     * a sign of poor tests. Normally, you don't need to reset your mocks, just
     * create new mocks for each test method.
     * <p>
     * Instead of reset() please consider writing simple, small and focused test
     * methods over lengthy, over-specified tests. <b>First potential code smell
     * is reset() in the middle of the test method.</b> This probably means
     * you're testing too much. Follow the whisper of your test methods:
     * "Please keep us small & focused on single behavior". There are several
     * threads about it on mockito mailing list.
     * <p>
     * The only reason we added reset() method is to make it possible to work
     * with container-injected mocks. See issue 55 (<a
     * href="http://code.google.com/p/mockito/issues/detail?id=55">here</a>) or
     * FAQ (<a href="http://code.google.com/p/mockito/wiki/FAQ">here</a>).
     * <p>
     ...
     *
     * // Impossible: the exception-stubbed foo() method is called so RuntimeException
     * // is thrown.
     ...
     * overriding exception-stubbing is very rare. Not to mention that in
     * general overridding stubbing is a potential code smell that points out
     * too much stubbing.
     * <p>
     * See examples in javadoc for {@link Mockito} class
     *
     ...
     * testing in order.
     * <p>
     * Also, you can create InOrder object passing only mocks that are relevant
     * for in-order verification.
     * <p>
     * InOrder verification is 'greedy'. You will hardly every notice it but if
     * you want to find out more search for 'greedy' on the Mockito <a
     * href="http://code.google.com/p/mockito/w/list">wiki pages</a>.
     * <p>
     * As of Mockito 1.8.4 you can verifyNoMoreInvocations() in order-sensitive
     * way. Read more: {@link InOrder#verifyNoMoreInteractions()}
     * <p>
     * See examples in javadoc for {@link Mockito} class
     *
     * @param mocks
     ...
     * FAQ: <a
     * href="http://code.google.com/p/mockito/wiki/FAQ">http://code.google
     * .com/p/mockito/wiki/FAQ</a>
     * <p>
     * In case of questions you may also post to mockito mailing list: <a
     * href="http://groups.google.com/group/mockito"
     * >http://groups.google.com/group/mockito</a>
     * <p>
     ...
     * <b>because Mockito validates the usage all the time...</b> but there is a
     * gotcha so read on.
     * <p>
     ...
     * verify(mock.execute());
     *
     * // Oups, someone has used EasyMock for too long and forgot to specify the method
     * // to verify:
     * verify(mock);
     * </pre>
     *
     * Mockito throws exceptions if you misuse it so that you know if your tests
     * are written correctly. The gotcha is that Mockito does the validation
     * <b>next time</b> you use the framework (e.g. next time you verify, stub,
     * call mock etc.). But even though the exception might be thrown in the
     * next test, the exception <b>message contains a navigable stack trace
     * element</b> with location of the defect. Hence you can click and find the
     * place where Mockito was misused.
     * <p>
     ...
     * about it not sooner than <b>next time</b> he used the framework. One more
     * benefit of having validateMockitoUsage() in &#064;After is that jUnit
     * runner will always fail in the test method with defect whereas ordinary
     * 'next-time' validation might fail the <b>next</b> test method. But even
     * though JUnit might report next test as red, don't worry about it and just
     * click at navigable stack trace element in the exception message to
     * instantly locate the place where you misused mockito.
     * <p>
     ...
     * location of defect. However, I would recommend validateMockitoUsage() if
     * you already have sufficient test infrastructure (like your own runner or
     * base class for all tests) because adding a special action to &#064;After
     * has zero cost.
     * <p>
     * See examples in javadoc for {@link Mockito} class
     */
     ...
     * maintainable code. If you cannot write a test in a simple way - refactor
     * the code under test.
     * <p>
     ...
     *
     * // Creates mock with different default answer, descriptive name and extra
     * // interfaces
     * Foo mock = mock(Foo.class,
     *         withSettings().defaultAnswer(RETURNS_SMART_NULLS).name(&quot;cool mockie&quot;).extraInterfaces(Bar.class));
     * </pre>
     *
     * {@link MockSettings} has been introduced for two reasons. Firstly, to
     * make it easy to add another mock settings when the demand comes.
     * Secondly, to enable combining different mock settings without introducing
     * zillions of overloaded mock() methods.
     * <p>
     * See javadoc for {@link MockSettings} to learn about possible mock
     * settings.
     * <p>
     *
